...
def error_wire(circuit_output):
    ...
    onetwo, onethree, twothree = circuit_output
    onetwo, onethree, twothree = (-onetwo+1)/2 ,(-onethree+1)/2,(-twothree+1)/2
    
    if onetwo==onethree and onetwo!=0:
        error_readout[1]=onetwo
        error_readout[0]=1-onetwo
    elif onetwo==twothree and onetwo!=0:
        error_readout[2]=onetwo
        error_readout[0]=1-onetwo
    elif onethree==twothree and onethree!=0:
        error_readout[3]=onethree
        error_readout[0]=1-onethree
...
def circuit(p, alpha, tampered_wire):
    ...
    return qml.expval(qml.PauliZ(0)@qml.PauliZ(1)), qml.expval(qml.PauliZ(0)@qml.PauliZ(2)), qml.expval(qml.PauliZ(1)@qml.PauliZ(2))
