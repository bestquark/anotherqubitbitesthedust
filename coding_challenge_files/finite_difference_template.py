...
def my_finite_diff_grad(params):
    ...
    for i in range(len(params)):
        # QHACK # 
        upchange, downchange = np.copy(params),np.copy(params)
        upchange[i],downchange[i] = upchange[i]+delta/2, downchange[i]-delta/2
        gradients[i] = (cost(upchange)-cost(downchange))/delta
        # QHACK #
