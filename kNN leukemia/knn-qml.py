...
def distance(A, B):
    ...
    dev = qml.device("default.qubit", wires=31)
    @qml.qnode(dev)
    def circuit(AA,BB):
      
        
        qml.AmplitudeEmbedding(features=[AA[0]*BB[0], AA[1]*BB[0], AA[0]*BB[1], AA[1]*BB[1]], wires=[1,2], normalize=True)
